...
class SplitSteeringSpeedYawEmbedder(torch.nn.Module):
    ...
    def forward(self, goal):
        ...
        if self.training and self.noise_amount > 0.0:
            noise_amount = torch.tensor(self.noise_amount, device=goal.device)
            goal = torch.sqrt((1.0 - noise_amount**2)) * goal + noise_amount * torch.randn_like(goal)

        speed, yaw_rate = goal[:, [0]], goal[:, [1]]
        ...
        sterring_embedding = speed_embedding + yaw_rate_embedding
        return sterring_embedding


class LinearGoalEmbedder(torch.nn.Module):
    """
    Generic linear embedding layer for goal signals.
    """
...
class STDiTGoalConditioned(STDiT):    
    ...
    def get_condition_embeddings(self, t, goal):
        """Conditioned on diffusion time and goal coordinate"""
        c = self.t_embedder(t) + self.goal_embedder(goal)
        return c
    ...
    def forward(self, target, context, t, frame_rate, goal):        
        num_frames_pred = target.size(1)

        c = self.get_condition_embeddings(t, goal)
