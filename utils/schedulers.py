...
class CosineSchedule(object):
    ...
    def __init__(self, optimizer, ref_lr, T_max, final_lr=0.0):
        self.optimizer = optimizer
        self.ref_lr = ref_lr
        self.final_lr = final_lr
        self.T_max = T_max
    ...
    def step(self):
        self._step += 1
        progress = self._step / self.T_max
        new_wd = self.final_lr + (self.ref_lr - self.final_lr) * 0.5 * (1.0 + math.cos(math.pi * progress))

        if self.final_lr <= self.ref_lr:
            new_wd = max(self.final_lr, new_wd)
        else:
            new_wd = min(self.final_lr, new_wd)
